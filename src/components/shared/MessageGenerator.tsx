...
const MessageGenerator: React.FC<MessageGeneratorProps> = ({ 
  ...
  }, [selectedLeadId, leads, message]);

  // Generate a basic message template based on lead info
  const generateBaseMessage = (lead: Lead) => {
    const firstName = lead.name.split(' ')[0];
    
    const baseTemplate = `Hi ${lead.name},

I came across your profile and was impressed by your work at ${lead.company || 'your company'} as ${lead.title || 'a professional'}. Your experience in the industry really stands out!

I'd love to connect and explore potential synergies between our organizations. We're helping companies similar to ${lead.company || 'yours'} with innovative solutions that might be of interest to you.
  ...
  const applyCustomPrompt = async (promptOverride?: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/generate-message', {
        ...
        body: JSON.stringify({ 
          baseMessage: message, // Use current message as base, not original template
